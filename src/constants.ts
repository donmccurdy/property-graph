...
import { type RefList, type RefLinkedList, type RefMap, type RefSet } from './refs.js';
...
export type RefListKeys<T> = { [K in keyof T]-?: T[K] extends RefList | RefLinkedList ? K : never }[keyof T];
export type RefListValue<List> = List extends RefList<infer V> | RefLinkedList<infer V> ? V : never;
...
export type RefCollectionValue<Collection> = Collection extends
	| RefList<infer T>
	| RefLinkedList<infer T>
	| RefSet<infer T>
	| RefMap<infer T>
	? T
	: never;
